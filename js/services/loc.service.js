...
const DB_KEY = 'locs'
var gSortBy = { rate: -1 }
var gFilterBy = { txt: '', minRate: 0, createdAt: -1 }
var gPageIdx
...
function query() {
    return storageService.query(DB_KEY)
        .then(locs => {
            ...
            if (gFilterBy.createdAt) {
                locs = locs.filter(loc => loc.createdAt >= gFilterBy.createdAt)
            }
...
function setFilterBy(filterBy = {}) {
    ...
    if (filterBy.createdAt !== undefined) gFilterBy.createdAt = filterBy.createdAt
    return gFilterBy
}
...
function getUpdatetStats() {
    return storageService.query(DB_KEY)
    .then(locs => {
        const locCountByUpdated = locs.reduce((map, loc) => {
            if (loc.updatedAt === loc.createdAt) map.never++
            else if (Date.now() - loc.updatedAt < 1000 * 60 * 60 * 24 ) map.today++
            else map.past++
            return map
        }, { today: 0, past: 0, never: 0 })
        locCountByUpdated.total = locs.length
        return locCountByUpdated
    })
}
...
function countDistances(givenLocation) {
    const places = utilService.loadFromStorage(DB_KEY)
    places.forEach((place) => {
        const distance = utilService.getDistance(place.geo, givenLocation, 'K') // 'K' for kilometers
        place.distanceFromGivenLocation = distance // Add distance as a new key in the place object
    })
    utilService.saveToStorage(DB_KEY, places)
}
